...
def test_udpates_manager_suffle_with_params():
    ...
    mocked_api.post.assert_called_once_with(url='profiles/1/updates/shuffle.json', data=data)
    eq_(updates, True)
...
def test_updates_manager_reorder():
    ...
    mocked_api.post.assert_called_once_with(url='profiles/1/updates/reorder.json', data=data)
